    ...
    <?php
        ...
        if(isset($_FILES['bukti_terima']) && $_FILES['bukti_terima']['error'] === UPLOAD_ERR_OK) {
            ...
            if(move_uploaded_file($file_tmp, $target_file)) {
                ...
                $update_status_kirim_query = "UPDATE status_kirim SET jenis_ongkir = '$jenis_ongkir', no_resi = '$no_resi' WHERE id_inv_ecat = '$id_inv_ecat'";
                mysqli_query($koneksi, $update_status_kirim_query);

                // Insert data ke tabel inv_bukti_terima
                $created_date = date('Y-m-d H:i:s'); 
                $insert_bukti_terima_query = "INSERT INTO inv_bukti_terima (id_bukti_terima, id_inv_ecat, bukti_terima, created_date) VALUES ('$BKTuuid', '$id_inv_ecat', '$file_content', '$created_date')";
                mysqli_query($koneksi, $insert_bukti_terima_query);

                // Update tabel inv_ecat
                $ongkir = preg_replace("/[^0-9]/", "", $ongkir);
                $update_inv_ecat_query = "UPDATE inv_ecat SET ongkir = '$ongkir' WHERE id_inv_ecat = '$id_inv_ecat'";
                ...
                header("Location: ../details_ecat_dikirim.php?id_inv_ecat=$id_inv_ecat");
                exit(); 
            } else {
