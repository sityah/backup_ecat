...
<?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $query_insert = "INSERT INTO status_kirim (id_inv_ecat, id_status_kirim, id_driver, id_ekspedisi, jenis_pengiriman, tgl_kirim, dikirim_oleh, penanggung_jawab) 
                    VALUES ('$id_inv_ecat', '$KRMuuid', '$id_driver', '$id_ekspedisi', '$jenis_pengiriman', '$tgl_kirim', '$dikirim_oleh', '$penanggung_jawab')";
            
            // Eksekusi query INSERT
            if(mysqli_query($koneksi, $query_insert)) {
                // Setelah berhasil melakukan INSERT, lakukan pembaruan ke tabel tb_spk_ecat
                $query_update = "UPDATE tb_spk_ecat SET status_spk_ecat = 'Dikirim' WHERE id_inv_ecat = '$id_inv_ecat'";
                
                // Eksekusi query UPDATE
                if(mysqli_query($koneksi, $query_update)) {
                    // Mengambil total spk dari tb_spk_ecat
                    $query_total_invoice = "SELECT SUM(total_spk_ecat) AS total FROM tb_spk_ecat WHERE id_inv_ecat = '$id_inv_ecat'";
                    ...
                    $query_update_total = "UPDATE inv_ecat SET total_inv_ecat = '$totalInvoice' WHERE id_inv_ecat = '$id_inv_ecat'";
                    ...
                    if($result_update_total) {
                        ...
                        $query_update_status_transaksi = "UPDATE inv_ecat SET status_transaksi = 'Dikirim' WHERE id_inv_ecat = '$id_inv_ecat'";
                        ...
                        if($result_update_status_transaksi) {
                            echo "<script>
                                    Swal.fire({
                                        ...
                                        window.location.href = '../spk_invoice_dicetak.php';
                                    });
                                </script>";
                            exit();
                        } else {
                            echo "ERROR: Gagal memperbarui status transaksi di inv_ecat: " . mysqli_error($koneksi);
                        }
                    } else {
                        echo "ERROR: Gagal memperbarui total invoice: " . mysqli_error($koneksi);
                    }
                } else {
                    echo "ERROR: Gagal memperbarui status SPK ECAT: " . mysqli_error($koneksi);
                }
            } else {
