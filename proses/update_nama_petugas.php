...
<?php
    ...
    if(isset($_POST['namaPetugas'], $_POST['spkId'])) {
        ...
        $query_spk = "UPDATE tb_spk_ecat SET petugas_ecat = '$namaPetugas', status_spk_ecat = 'Siap Kirim' WHERE id_spk_ecat = '$spkId'";
        $result_spk = mysqli_query($koneksi, $query_spk);

        // Mengambil total harga dari transaksi_produk_ecat
        $query_total_harga = "SELECT SUM(total_harga) AS total FROM transaksi_produk_ecat WHERE id_spk = '$spkId'";
        ...
        $query_update_total = "UPDATE tb_spk_ecat SET total_spk_ecat = '$totalHarga' WHERE id_spk_ecat = '$spkId'";
            ...
            echo "<script>
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href = '../spk_dalam_proses.php';
                        }
                    });
                </script>";
        } else {
