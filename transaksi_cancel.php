                                ...
                                <table class="table" id="spk_ecat" style="width:100%">
                                    ...
                                    <tbody>
                                        <?php
                                            ...
                                            $query = "SELECT 
                                                        tb.id_inv_ecat,
                                                        GROUP_CONCAT(tb.no_spk_ecat SEPARATOR ', ') AS no_spk_combined,
                                                        CONCAT('<strong>(', 
                                                            (SELECT DISTINCT inv.no_inv_ecat 
                                                                FROM inv_ecat inv 
                                                                WHERE inv.id_inv_ecat = tb.id_inv_ecat 
                                                                LIMIT 1),'</strong>)') AS inv_numbers,
                                                        tb.tgl_spk_ecat,
                                                        tb.id_spk_ecat,
                                                        tb.notes,
                                                        sales.nama_sales
                                                    FROM tb_spk_ecat AS tb
                                                    JOIN tb_sales_ecat AS sales ON tb.id_sales = sales.id_sales
                                                    LEFT JOIN inv_ecat AS inv ON tb.id_inv_ecat = inv.id_inv_ecat
                                                    WHERE tb.status_spk_ecat = 'Cancel'
                                                    GROUP BY tb.id_inv_ecat";
                                            $result = mysqli_query($koneksi, $query);
                                            ...
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                echo "<td>
                                                        <a href='details_transaksi_cancel.php?id_spk_ecat=" . $row['id_spk_ecat'] . "' class='btn btn-info btn-sm mt-2'>
